...
function makeScene(setScore) {
  return class Match3Scene extends Phaser.Scene {
    ...
    causesMatch(x, y, t) { return (x>=2 && this.board[y][x-1]===t && this.board[y][x-2]===t) || (y>=2 && this.board[y-1][x]===t && this.board[y-2][x]===t); }
    place(x, y, t) {
      this.board[y][x] = t;
    ...
    onUp(pointer) {
      ...
      const {x:sx, y:sy} = this.selected; this.selected=null;
      if(x===sx && y===sy) return;
    ...
    trySwap(x1,y1,x2,y2) {
      ...
      if(this.hasMatches()) this.animateSwap(x1,y1,x2,y2,true);
      ...
      this.time.delayedCall(120, ()=>{
        for(let x=0;x<GRID;x++){
          ...
          for(let y=GRID-1;y>=0;y--){
            if(this.board[y][x]!==-1){
              if(y!==writeY){ this.board[writeY][x]=this.board[y][x]; const s=this.spriteAt(x,y); if(s){ s.setData('y',writeY); const {px,py}=this.gridToXY(x,writeY); this.tweens.add({targets:s,y:py,duration:120}); } }
              writeY--;
            }
          }
          for(let y=writeY;y>=0;y--){
            const t=Math.floor(Math.random()*TYPES); this.board[y][x]=t;
            const {px,py}=this.gridToXY(x,y);
