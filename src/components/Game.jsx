...
import { createSession, startGame, submitScore } from "../lib/api.js";
...
function makeScene(setScore) {
  return class Match3Scene extends Phaser.Scene {
    ...
    resetBoard() {
      ...
      for(let y=0;y<GRID;y++){
        for(let x=0;x<GRID;x++){
          let t; do { t=Math.floor(Math.random()*TYPES); } while(this.causesMatch(x,y,t));
          this.place(x,y,t);
        }
      }
    ...
    collectMatches(){
      ...
      const vmatched=[]; for(let x=0;x<GRID;x++){ let run=1; for(let y=1;y<GRID;y++){ if(this.board[y][x]===this.board[y-1][x]) run++; else{ if(run>=3) vmatched.push({x,y0:y-run,y1:y-1}); run=1; } } if(run>=3) vmatched.push({x,y0:GRID-run,y1:GRID-1}); }
